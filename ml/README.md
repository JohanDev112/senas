...
legacy `mediapipe.solutions.hands` que usa `extract_features.py`.
...
# 3. Con el pipeline validado, correr sobre el dataset completo (~70 min)
uv run --python ml/.venv ml/extract_features.py

# 4. Entrenar (unos minutos, la red es chica)
uv run --python ml/.venv ml/train.py

# 5. Exportar a JSON (queda copiado directo en apps/mobile/assets/models/)
...
las mismas 15 features en tiempo real.
...
15 -> 32 -> 32 -> 21 no vale la pena cargar un runtime de inferencia
